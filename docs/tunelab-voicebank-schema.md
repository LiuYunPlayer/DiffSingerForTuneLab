...
| `voicing` | 混合 delta | `[0, 1]` | `1` | 1=不变，0 = 触底 −96 dB | `Delta: x − 48(1−y)/(2−y) − (x+72)(1−y)¹²` → clamp `[-96,0]`dB，见下 |
...
**voicing 的公式是对 OpenUtau 基准的有意偏离**（其余三条忠实移植 OpenUtau `VarianceDeltaFunctions`，系数按小数刻度 ×100）。OpenUtau 的 voicing 满偏只有 −12 dB，永远够不着静音底——无法做纯气声/耳语段。改用「饱和有理项 + 十二次幂跳水项」：`实参 = 预测 − 48(1−y)/(2−y) − (预测+72)(1−y)¹²`，按三个锚定需求逐点定形：
