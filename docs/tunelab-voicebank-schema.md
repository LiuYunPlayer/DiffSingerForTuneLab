...
- 边角：预测 < −72 dB 的帧（本就近静音）跳水项系数变负、中段轻微越过 −96；用户值越界（宿主数据层无量程硬契约：锚点+默认值合成、跨引擎同 key 复用、手改工程）会使偶次幂/分母变号产生荒谬值——两者皆由合成期双 clamp 兜住（输入钳 `EditMin/Max`、输出钳声学值域，见 `CombineVariance`）。
...
variance 回显轨（`SynthesizedParameters`）是**实参（预测 + 用户 delta 合成、clamp 后）、绝对值、真实 dB**（energy/breath/voicing `[-96,0]`、tension `[-10,10]`），与编辑轨的归一化 delta **不同质、不强行同单位**——与 pitch 回显同语义：所见即喂给声学的值（编辑效果与 clamp 撞墙均可见）。mulaw 声库的 voicing 回显亦恒为 dB（取三明治的 dB 语义中间值，见 §14.2）——回显单位不随模型编码漂移。理由：回显**不存进 part**、每次合成现算 → **零跨引擎复用风险**；且 dB 本就是公共单位。该 dB 值域同时被合成期 clamp 复用。
